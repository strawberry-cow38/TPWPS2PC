...
Updated: 2026-09-24 UTC. Current priority: native entrance-group and departure-pressure guest lifecycle. Bus integration and requested presentation smoothing are shipped; the research branch now has an opt-in incoming controller through actual bus births, queues and fees, but native route-service/readiness, fee overrides and departure/rejection handling remain release blockers. Destination scoring, relief, shop walking and Bouncy's explicit hide-list activation are shipped; do not redo them. Exact commits, evidence and remaining boundaries are in progress.md. Cow tools owns the active animated-texture/thought-bubble rendering requests. The original `0db3678` starting point is historical.
