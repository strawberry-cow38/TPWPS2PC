...
cp "$sim" "$work/sim.cs"
cp "$vm" "$work/vm.cs"
restore() { cp "$work/sim.cs" "$sim"; cp "$work/vm.cs" "$vm"; }
trap restore EXIT
...
run restored
if [[ $# == 2 ]]; then
    dotnet build game/TPWPS2Viewer.csproj > "$work/game-build.log" 2>&1
    TPW_PS2_DISC="$disc" "$2" --headless --path game res://tests/VisitorAudit.tscn > "$work/geometry.log" 2>&1
    code=0
    TPW_PS2_DISC="$disc" "$2" --headless --path game res://tests/VisitorAudit.tscn -- --mutate-position > "$work/geometry-mutant.log" 2>&1 || code=$?
    if [[ $code != 2 ]] || ! rg -F 'Ada rendered position identity at 100ms' "$work/geometry-mutant.log"; then
        cat "$work/geometry-mutant.log"; echo "Expected geometry identity failure missing (exit $code)" >&2; exit 1
    fi
    echo "TEETH rendered_position: expected failure, exit $code"
fi
